...
public class SimpleEventsActivity extends Activity {
	...
	private void loadConfiguration() {
		...
		sendBP = ((CheckBox)findViewById(R.id.cbBP)).isChecked();
		sendWeight = ((CheckBox)findViewById(R.id.cbWeight)).isChecked();
		sendAcc = ((CheckBox)findViewById(R.id.cbAcc)).isChecked();
		
		/* random? */
		randomHR = ((CheckBox)findViewById(R.id.cbHRrandom)).isChecked();
		randomBP = ((CheckBox)findViewById(R.id.cbBPrandom)).isChecked();
		randomWeight = ((CheckBox)findViewById(R.id.cbWeightrandom)).isChecked();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.simple_events_activity);
		...
		getApplicationContext().registerReceiver(mReadingEventReceiver, new IntentFilter(SensorReadingEvent.BLOOD_PRESSURE));
    }
			...
			public void run() {
				...
				if(sendBP) {
					if(randomBP) {
						...
						evt = new BloodPressureEvent(TAG+"_BP_"+round, getGMTTime(),
								"EventGenerator", "EventGenerator", round+"",
								Integer.valueOf(((EditText)findViewById(R.id.etBPsys)).getText().toString()),
								Integer.valueOf(((EditText)findViewById(R.id.etBPdia)).getText().toString()),								
								getRandomNumber(55, 180), "mmHG");
					}
					injectEvent(evt);
				...
				if(sendWeight) {
					if(randomWeight) {
						...
						evt = new WeightEventInKg(TAG+"_Weight_"+round, getGMTTime(),
								"EventGenerator", "EventGenerator", round+"",
								Integer.valueOf(((EditText)findViewById(R.id.etWeight)).getText().toString())*10);
					}
					injectEvent(evt);
    ...
    public void onClickWeightRandom(View v) {
    	if(((CheckBox)findViewById(R.id.cbWeightrandom)).isChecked()) {
    ...
    public void onClickBPRandom(View v) {
    	if(((CheckBox)findViewById(R.id.cbBPrandom)).isChecked()) {
    		((EditText)findViewById(R.id.etBPsys)).setEnabled(false);
    		((EditText)findViewById(R.id.etBPdia)).setEnabled(false);
    	} else {
    		((EditText)findViewById(R.id.etBPsys)).setEnabled(true);
    		((EditText)findViewById(R.id.etBPdia)).setEnabled(true);
    	}
    }
